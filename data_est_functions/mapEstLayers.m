...
if isfield(Maps,'nrn_phis_all')
   nrn_phis_all = Maps.nrn_phis_all;    
    ...
    if strcmp(map_type,'polarization') 
        ...
        map_filename = sprintf('%s_ls_%g_E_%s_P_%s_%s_%s_%s_%s',Maps.model_prefix,Maps.layer_set_num,...
                Maps.Efield_name,pop_str,method,...
                func_str,Maps.comp_type,Maps.sectype);        
    else 
        map_filename = sprintf('%s_ls_%g_E_%s_P_%s',model_prefix,layer_set_num,...
                Maps.Efield_name,pop_str);       
    end    
    ...
    case 'polarization'
        ...
        for li = 1:num_layers
            ...
            for cj = 1:num_cells_in_layer                
                ...
                deltaVms{cell_cnt} = zeros(layersE(li).num_elem,num_pops);
                % transform E to local neuron space (s-d axis aligned to [0 0 1])
                deltaVm_int_grid_ij = deltaVm_int_grids{cell_ind};                                
                if size(Ei,3) == num_cells_in_layer
                    Eic = Ei(:,:,cj);
                    cell_normalsc = cell_normals(:,:,cj);
                else
                    Eic = Ei;
                    cell_normalsc = cell_normals;
                end
                for pop = 1:num_pops                    
                    [~,~,pop_ind] = intersect(nrn_pop_names{pop},Maps.nrn_pop_names);
                    Ei_loc = zeros(size(Eic));
                    for posk = 1:size(Ei,1)
                        Ei_loc(posk,:) = reorientEfield(cell_normalsc(posk,:),...
                                                        nrn_phis_all{pop_ind}{li}{cj}(posk),Eic(posk,:));
                    end
